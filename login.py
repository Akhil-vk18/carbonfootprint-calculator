...
def login():
    ...
    def sign_up_with_email_and_password(email, password, username=None, return_secure_token=True):
        try:
            ...
            r = requests.post(rest_api_url, params={"key": os.getenv('FIREBASE_API_KEY')}, data=payload)
            try:
                return r.json()['email']
            except:
                st.warning(r.json())
    ...
    if not st.session_state["signedout"]: # only show if the state is False, hence the button has never been clicked
        choice = st.selectbox('Login/Signup', ['Login', 'Sign up'], key='login_signup_selectbox_unique_2')
        email = st.text_input('Email Address', key='email_input_unique_2')
        password = st.text_input('Password', type='password', key='password_input_unique_2')

        if choice == 'Sign up':
            username = st.text_input("Enter your unique username", key='username_input_unique_2')

            if st.button('Create my account', key='create_account_button_unique_2'):
                user = sign_up_with_email_and_password(email=email, password=password, username=username)
                st.success('Account created successfully!')
                st.markdown('Please Login using your email and password')
                st.balloons()
        else:
            if st.button('Login', key='login_button_unique_2'):
    ...
    if st.session_state.signout:
        ...
        if st.button('Sign out', on_click=t, key='signout_button_unique_2'):
            st.session_state.signout = False
            st.session_state.signedout = False
